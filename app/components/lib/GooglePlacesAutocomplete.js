...
export default class GooglePlacesAutocomplete extends Component {
    ...
    buildRowsFromResults = (results, hasFetched) => {
        let res = [];
        ...
        if (results.length === 0 && hasFetched) {
            res.push({
                description: this.props.noResultsLabel,
                isNotFound: true
            });
        }
        return [...res, ...results];
            ...
            request.onreadystatechange = () => {
                ...
                if (request.status === 200) {
                    ...
                    if (!this.props.onFail) {
                        ToastAndroid.show('Une erreur inconnue est survenue. Veuillez réessayer plus tard.', ToastAndroid.LONG);
                    } else {
                        this.props.onFail();
                    }
                }
                ...
                if (request.status === 200) {
                    ...
                    if (typeof responseJSON.results !== 'undefined') {
                        if (this._isMounted === true) {
                            ...
                            this.setState({
                                dataSource: this.buildRowsFromResults(results, true),
                            });
                        }
                    }
                    ...
                    ToastAndroid.show('Une erreur inconnue est survenue. Veuillez réessayer plus tard.', ToastAndroid.SHORT);
                }
            };
    ...
    _renderRowData = (rowData) => {
        ...
        return (
            <Text
                style={[{flex: 1}, this.props.suppressDefaultStyles ? {} : defaultStyles.description, this.props.styles.description, rowData.isPredefinedPlace ? this.props.styles.predefinedPlacesDescription : {}, rowData.isNotFound ? this.props.styles.isNotFoundDescription : {}]}
        ...
        return (
            ...
            >
                <Image
                    ...
                    resizeMode='contain'
                    source={require('../../../assets/img/powered_by_google_on_white.png')}
                />
            </View>
        );
    }
    ...
    _shouldShowPoweredLogo = () => {
        ...
        for (let i = 0; i < this.state.dataSource.length; i++) {
            ...
            if (!row.hasOwnProperty('isCurrentLocation') && !row.hasOwnProperty('isPredefinedPlace') && !row.hasOwnProperty('isNotFound')) {
                return true
            }
        }
...
GooglePlacesAutocomplete.defaultProps = {
    ...
    getDefaultValue: () => '',
    timeout: 20000,
    onTimeout: () => {ToastAndroid.show('Timeout error. Le serveur est trop lent à répondre.', ToastAndroid.LONG)},
    query: {
        key: 'missing api key',
        language: 'en',
        types: 'geocode',
